...
baath_ll <- function(rho,sigma,d){
  ...
  # the log likelihood
  ll <- -lchoose(2*rho + sigma,d) +  matrixStats::logSumExp(log_summation_terms)
  
  return(ll)
}


# function to calculate grid of prior/posterior likelihoods
...
baath_grid <- ll_grid(30, 20, 11, baath_ll)

ggplot() +
  geom_point(data = baath_grid %>% filter(!is.infinite(ll)), aes(x = p, y = s, color =exp(ll)), size =4) +
  geom_point(data = baath_grid %>% filter(is.infinite(ll)), aes(x = p, y = s,color=ll),shape = 1, size =4) +
  # scale_color_gradientn(colours = c( infreq_palette["lightblue"], infreq_palette["orange"])) +
  scale_color_gradientn(colours = wesanderson::wes_palette("Zissou1", 1000, type = "continuous")) +
  labs(
    title = "Bååth's likelihood",
    x = "2ρ - Total socks that are in pairs",
    y = "σ - Singleton socks",
    colour = "BLikelihood"
  ) +
